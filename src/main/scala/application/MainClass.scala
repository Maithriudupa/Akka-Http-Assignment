...
import route.DeleteRoute.deleteRoute
import route.UpdateRoute.updateRoute
import route.PostRoute.postRoute
...
import crud.GetUserRequestHandler
...
object Route extends App  with UserJsonProtocol with SprayJsonSupport{
  ...
  val requestHandler = new GetUserRequestHandler()
      ...
      pathPrefix("api" ) {
        get {
          ...
          path("users" / Segment) { userName =>
            val users = Future {
              requestHandler getAllUser(userName)
            }
            ...
            path("user" / IntNumber) { userId =>
              val users = Future {
                requestHandler getAllUserBasedOnId(userId)
              }
              val result = users.map(user =>
